    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="flex flex-col gap-1.5">
          ...
          <select value={form.categoryId} onChange={set('categoryId')} className="bg-gray-800/60 border border-gray-700/60 rounded-xl px-3.5 py-2.5 text-sm text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 hover:border-gray-600 transition-all">
        ...
        {initial && (
          <div className="flex flex-col gap-1.5">
            ...
            <select value={form.status} onChange={set('status')} className="bg-gray-800/60 border border-gray-700/60 rounded-xl px-3.5 py-2.5 text-sm text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 hover:border-gray-600 transition-all">
      ...
      <div className="p-8 space-y-4">
        <div className="relative max-w-sm">
          ...
          <input
            ...
            className="w-full bg-gray-900/60 border border-gray-800/60 rounded-xl pl-10 pr-4 py-2.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all"
          />
        </div>
                  ...
                  <tr key={p.productId} className="hover:bg-white/[0.02] transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="p-2 bg-gray-800/60 rounded-lg ring-1 ring-gray-700/30"><Package size={14} className="text-gray-500" /></div>
                        <div>
                          <p className="text-sm font-medium text-white">{p.name}</p>
                          {p.description && <p className="text-xs text-gray-600 truncate max-w-xs">{p.description}</p>}
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 text-sm text-gray-600 tabular-nums">{new Date(p.createdAt).toLocaleDateString()}</td>
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        {p.referenceLink && <a href={p.referenceLink} target="_blank" rel="noopener noreferrer" className="p-1.5 rounded-lg text-gray-500 hover:text-blue-400 hover:bg-blue-500/10 transition-colors"><ExternalLink size={14} /></a>}
                        <button onClick={() => { setSelected(p); setModal('edit'); }} className="p-1.5 rounded-lg text-gray-500 hover:text-white hover:bg-white/10 transition-colors"><Pencil size={14} /></button>
                        <button onClick={() => setDeleting(p)} className="p-1.5 rounded-lg text-gray-500 hover:text-red-400 hover:bg-red-500/10 transition-colors"><Trash2 size={14} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
