            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${
                filter === f
                  ...
                  : 'bg-gray-900/60 text-gray-500 hover:text-white hover:bg-white/5'
              }`}
            >
        ...
        {isLoading ? <LoadingSpinner /> : promotions?.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {promotions?.map(promo => (
              <div key={promo.promoId} className={`bg-gray-900/60 border rounded-2xl p-5 hover:border-gray-700/60 transition-all duration-200 group ${promo.isActive ? 'border-emerald-500/20' : 'border-gray-800/60'}`}>
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className={`p-2.5 rounded-xl ring-1 ${promo.isActive ? 'bg-emerald-500/10 ring-emerald-500/20' : 'bg-gray-800/60 ring-gray-700/30'}`}>
                      <Percent size={16} className={promo.isActive ? 'text-emerald-400' : 'text-gray-500'} />
                    </div>
                    <div>
                      <p className="text-white font-semibold text-sm">{promo.productName ?? `Product #${promo.productId}`}</p>
                      <Badge color={promo.isActive ? 'green' : 'gray'}>{promo.isActive ? 'Active' : 'Inactive'}</Badge>
                    </div>
                  </div>
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => { setSelected(promo); setModal('edit'); }} className="p-1.5 rounded-lg text-gray-500 hover:text-white hover:bg-white/10 transition-colors"><Pencil size={14} /></button>
                    <button onClick={() => setDeleting(promo)} className="p-1.5 rounded-lg text-gray-500 hover:text-red-400 hover:bg-red-500/10 transition-colors"><Trash2 size={14} /></button>
                  </div>
                </div>
                <div className="text-3xl font-bold text-white mb-3 tabular-nums">{promo.discountPercentage ?? 0}<span className="text-lg text-gray-500">%</span></div>
