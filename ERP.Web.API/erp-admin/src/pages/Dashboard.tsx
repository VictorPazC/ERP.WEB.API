        ...
        <div className="bg-white dark:bg-gray-900/60 border border-gray-200 dark:border-gray-800/60 rounded-2xl p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-gray-900 dark:text-white font-semibold text-sm">Estimated Profit per Product</h3>
                <p className="text-xs text-gray-500 dark:text-gray-600">Top performing products by margin</p>
              </div>
            </div>
            <span className="text-lg font-bold text-gray-900 dark:text-white tabular-nums">${totalProfit.toFixed(2)}</span>
          </div>
          <div className="space-y-3">
            {inventory?.slice(0, 8).map((inv, i) => (
              <div key={inv.inventoryId} className="flex items-center gap-2 sm:gap-3 group">
                <span className="text-[11px] text-gray-400 dark:text-gray-600 w-4 sm:w-5 tabular-nums flex-shrink-0">{i + 1}</span>
                <span className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 w-20 sm:w-36 truncate flex-shrink-0">{inv.productName ?? `Product #${inv.productId}`}</span>
                ...
                <span className="text-xs sm:text-sm font-semibold text-gray-900 dark:text-white w-16 sm:w-24 text-right tabular-nums flex-shrink-0">
                  ${inv.estimatedProfit.toFixed(2)}
                </span>
              </div>
            ))}
            {(!inventory || inventory.length === 0) && (
              <p className="text-gray-500 dark:text-gray-600 text-sm text-center py-8">No inventory data yet</p>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4">
          <div className="bg-white dark:bg-gray-900/60 border border-gray-200 dark:border-gray-800/60 rounded-2xl p-4 sm:p-6">
            ...
            <div className="space-y-1">
              ...
              {activePromotions === 0 && <p className="text-gray-500 dark:text-gray-600 text-sm text-center py-8">No active promotions</p>}
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-900/60 border border-gray-200 dark:border-gray-800/60 rounded-2xl p-4 sm:p-6">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                <h3 className="text-gray-900 dark:text-white font-semibold text-sm">Low Stock Alert</h3>
                <p className="text-xs text-gray-500 dark:text-gray-600">Products below 10 units</p>
              </div>
            </div>
            <div className="space-y-1">
              {inventory?.filter(i => i.currentStock < 10).slice(0, 5).map(i => (
                <div key={i.inventoryId} className="flex items-center justify-between py-2.5 px-3 rounded-xl hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors gap-2">
                  <span className="text-sm text-gray-700 dark:text-gray-300 truncate min-w-0">{i.productName ?? `Product #${i.productId}`}</span>
                  <Badge color={i.currentStock === 0 ? 'red' : 'yellow'}>{i.currentStock} units</Badge>
                </div>
              ))}
              {!inventory?.some(i => i.currentStock < 10) && (
                <p className="text-gray-500 dark:text-gray-600 text-sm text-center py-8">All products well stocked</p>
              )}
            </div>
          </div>
        </div>
